...
__attribute__((visibility("default"))) aclnnStatus aclnnNotifyDispatchGetWorkspaceSize(
    ...
    char *commGroup, int32_t rankSize, int32_t rankId, int32_t localRankSize, int32_t localRankId,
    int32_t round, int32_t perRoundTokens, const aclTensor *sendDataOffset, const aclTensor *recvData,
    const aclTensor *recvCount, const aclTensor *recvOffset, const aclTensor *expertGlobalOffset,
    const aclTensor *srcrankInExpertOffset, const aclTensor *rInSrcrankOffset, const aclTensor *totalRecvTokens,
    const aclTensor *maxBs, const aclTensor *recvTokensPerExpert, uint64_t *workspaceSize, aclOpExecutor **executor);
